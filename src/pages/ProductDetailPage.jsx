...
// base API URL (can be overridden by VITE_API_BASE env var)
...
const ProductDetailPage = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/products/${id}?user_id=${user.uid}`,
        {
          method: "DELETE",
        }
      );
  ...
  const extractKeywords = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/products/${id}/keywords`,
        { method: "POST" }
      );
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/products/${id}/analytics`
      );  
